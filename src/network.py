...
class NetworkRPL:
    ...
    def __init__(self,no_of_node=30,initial_node_power=50,dist_range=[0,50],ip='127.0.0.1',start_port=8000,print_func=print):
        ...
        for i in range(no_of_node):
            ...
            self.nodes['%s:%s'%addr] = NodeRPL(addr,initial_node_power,dist_range,print_func)
    ...
    def plt_energy_stat(self,ax,state='max_state'):
        ...
        ax.bar3d(x,y, b, 0.05,0.05 , z, shade=True,color=c, alpha=0.2)
    ...
    def plt_msg_delivery_stat(self,ax,state='max_state'):
        ...
        ax.bar3d(x,y, b,0.5/self.no_of_node,5/self.no_of_node, z, shade=True,color=c, alpha=0.2)
...
class NetworkAODV:
    ...
    def __init__(self,no_of_node=30,initial_node_power=50,dist_range=[0,50],ip='127.0.0.1',start_port=8000,print_func=print):
        ...
        for i in range(no_of_node):
            addr = (ip,start_port+i)
            self.nodes['%s:%s'%addr] = NodeAODV(addr,initial_node_power,dist_range,print_func)
            self.nodes['%s:%s'%addr].start()
        self.set_dest(addr)
        # Initilize neighbour
        self.init_neighbour()
        ...
        while count != no or no==-1: # -1 for infinite
            for node in self.nodes:
                if node != self.dest:
                    sent = False
                    self.init_neighbour()
                    self.nodes[node].send_user_message(self.dest,'PING')
                    # Check whether msg reaches to dest
                    for _ in range(self.MAX_ATTEMPT):
                        if node in self.nodes[self.dest].msg_box:
                            if not self.nodes[self.dest].msg_box[node][-1]['is_read']:
    ...
    def plt_dest_connection(self,ax):
        ...
        self.init_neighbour()
        for node in self.nodes.values():
            if self.dest in node.routing_table:
                node.routing_table[self.dest]['Status'] = 0
        for node in self.nodes.values():
            if node.node_id != self.dest:
                node.send_rreq(self.dest)
                for _ in range(self.MAX_ATTEMPT):
                    if self.dest in node.routing_table:
                        if node.routing_table[self.dest]['Status']:
                            route = node.routing_table[self.dest]
